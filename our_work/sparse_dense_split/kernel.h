...
__global__ void dp_comp_kernel_COO(int const* __restrict__ row_ind, int const* __restrict__ col_ind, float *val, const float * __restrict__ u, const float * __restrict__ v, 
  ...
  int *no_block_tile, int max_active_block, int tile_size, int max_active_row){
    ...
    __shared__ float sh_c[90*32];
    __shared__ float sh_r[90*32];
...
//                     //sm += u[row*k+t] * v[col*k+t];
//                     // sm += u[t*n_rows+row] * v[t*n_cols+cc];
//                     sm += u[t*n_rows+active_row_] * v[t*n_cols+col];//sh_c[t* sh_tile + sh_col]; 
