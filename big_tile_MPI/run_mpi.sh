...
for K in 32 64 128 256 512 1024; do
	log1=`mpiexec -n 1 -ppn 1 $bin $dataset $K 256 9999999`
	echo $dataset, $K, $log1 >> new_mpi_data_1node.csv
done

for K in 32 64 128 256 512 1024; do
	log1=`mpiexec -n 2 -ppn 1 $bin $dataset $K 256 9999999`
	echo $dataset, $K, $log1 >> new_mpi_data_2node.csv
done

# dataset=~/israt/graph_data/pubmed_row_sorted.mtx
# for K in 32 64 128 256 512 1024; do
# 	log2=`mpiexec -n 2 -ppn 1 $bin $dataset $K 256 9999999`
# 	echo $dataset, $K, $log2 >> mpi_data_2node.csv
# done