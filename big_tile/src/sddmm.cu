...
// sddmm_GPU(lastIdx_tile, count_actv_row, max_active_block, new_nnz, n_tile_c, n_tile_r, max_active_row, 
//     new_rows, new_cols, lastIdx_block_tile, W, H );
  
void sddmm_GPU(int *lastIdx_tile, int * count_actv_row, int &max_active_block, 
    const long new_nnz, const int n_tile_c, const int n_tile_r, int max_active_row,
    int * new_rows, int* new_cols, int* lastIdx_block_tile, float *W, float* H,
    float* p_ind, int * tiled_ind, int *active_row, const Matrix S ){
   ...
   //***********Starting GPU****************
    ...
    checkCuda(cudaMalloc((void**)&d_active_row, n_tile_c * max_active_row * sizeof (int)),4);
    //checkCuda(cudaMalloc((void**)&d_passive_row, n_tile_c*n_rows * sizeof (int)),4);
    checkCuda(cudaMalloc((void**)&d_lastIdx_block_tile, n_tile_c*max_active_block * sizeof (int)),4);
    ...
    int sum =0 ;
    for (int i = 0; i < n_tile_c; ++i){
        checkCuda(cudaMemcpy(d_active_row+sum, &(active_row[i*S.n_rows]), count_actv_row[i] * sizeof (int), cudaMemcpyHostToDevice),4); 
    ...
    int n_tile = S.n_cols/tile_sizeX + 1;
    ...
    for (int tile = 0; tile < n_tile; ++tile){
        ...
        // cout << "tile "<<tile<<" S.nnz "<< nnz_tile << " grid "<<grid.x <<" "<<count_actv_row[tile]<< endl;
        for (int t_st = 0; t_st < k ; t_st +=k_slice){
            comp_kernel_COO<<<grid,block, 0, stream[0]>>>(d_row_ind, d_col_ind, d_val, d_W, d_H, 
            S.nnz, S.n_rows, S.n_cols, k, lastIdx_tile[tile], lastIdx_tile[tile+1], &(d_lastIdx_block_tile[(tile)*max_active_block]), 
...
void sddmm_CPU_CSR(int * row_ptr, int *col_ind, float * val_ind, float * W, float *H, float * p_ind, const Matrix S){
       // reduction(+:rmse)
    long tot =0 ;
...
void sddmm_CPU_COO(vector<int> row_ind, vector<int> col_ind, vector<float> val_ind, float * W, float *H, float * p_ind, const Matrix S){
       // reduction(+:rmse)
...
void preprocessing(const Matrix S){
    ...
    float *p_ind = new float[S.nnz];
   
    float *W = new float[S.n_rows*k];
    float *W_t = new float[S.n_rows*k];
    float *H = new float[S.n_cols*k];
    float *H_t = new float[S.n_cols*k];
    int n_tile_c = S.n_cols/tile_sizeX + 1;
    int n_tile_r = S.n_rows/tile_sizeY + 1;
    int max_active_block = (S.n_rows/actv_row_size+1);
    int *count_actv_row = new int[n_tile_c];
    int *lastIdx_tile = new int[n_tile_c+1];
    int *lastIdx_block_tile = new int[(n_tile_c+1) * (S.n_rows/actv_row_size+1)];
    ...
    int *new_rows = new int[S.nnz + n_tile_c * BLOCKSIZE - 1];
    int *new_cols = new int[S.nnz + n_tile_c * BLOCKSIZE - 1];
    float *new_vals = new float[S.nnz + n_tile_c * BLOCKSIZE - 1];
    int *row_holder = new int[S.n_rows];
    int *tiled_ind = new int [S.nnz + n_tile_c * BLOCKSIZE - 1];
    int *active_row = new int[n_tile_c * S.n_rows];
    int *passive_row = new int[n_tile_c * S.n_rows];
    ...
    //comp_bin(n_bin, count, S.n_rows, row_ptr, S.nnz);
    int max_active_row=0;

    max_active_row = rewrite_matrix_1D(row_ptr, S.rows, S.cols, S.vals, new_rows, new_cols, new_vals, S.nnz, S.n_rows, S.n_cols, 
    ...
    // rewrite_col_sorted_matrix(row_ptr, rows, cols, vals, new_rows, new_cols, new_vals, S.nnz, S.n_rows, S.n_cols, 
    //     tile_sizeX, tiled_ind, lastIdx_tile, BLOCKSIZE, new_nnz);
    double t0 = seconds();
    // sddmm_CPU_CSR(row_ptr, cols, vals, W, H, p_ind);
    sddmm_CPU_COO(S.rows, S.cols, S.vals, W, H, p_ind, S);
    
    sddmm_GPU(lastIdx_tile, count_actv_row, max_active_block, new_nnz, n_tile_c, n_tile_r, max_active_row, 
    new_rows, new_cols, lastIdx_block_tile, W, H, p_ind, tiled_ind, active_row, S );
