...
void sddmm_GPU(int *lastIdx_tile, int * count_actv_row, int &max_active_block, 
    ...
    float* p_ind, int * tiled_ind, int *active_row ){
...
void preprocessing(int *rows, int *cols, float* vals){
    ...
    int *active_row = new int[n_tile_c * n_rows];
    int *passive_row = new int[n_tile_c * n_rows];
    ...
    sddmm_GPU(lastIdx_tile, count_actv_row, max_active_block, new_nnz, n_tile_c, n_tile_r, max_active_row, 
    new_rows, new_cols, lastIdx_block_tile, W, H, p_ind, tiled_ind, active_row );
