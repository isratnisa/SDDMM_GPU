...
void rewrite_sp_matrix(int * row_ptr, int * row_ind, int *col_ind, float * val_ind, 
    int *sp_rows, int *sp_cols, float * sp_vals, long nnz, long n_rows, long n_cols,
    int TS, int *tiled_ind, int * lastIdx_tile,  int block, long &new_nnz){
    int th = 6;
    long sp_idx = 0,dp =0, idx =0;
    int n_tile = n_cols/TS + 1, tile_no=0;
    ...
    for(int tile_lim = TS; tile_lim <= (n_cols+TS-1); tile_lim+=TS){ 
        ...
        for(int r = 0; r <n_rows; ++r){ 
            ...
            while(col_ind[idx] < tile_lim-(TS/2) && idx < row_ptr[r+1]){
                t1_ct++;
                idx++;
            }
            if(t1_ct <= th){
                idx -= t1_ct;
                for (int i = 0; i < t1_ct; ++i){
                    tiled_ind[sp_idx+i] = idx+i;
                    sp_rows[sp_idx+i] = row_ind[idx+i];
                    sp_cols[sp_idx+i] = col_ind[idx+i];
                    sp_vals[sp_idx+i] = val_ind[idx+i];
                }
                sp_idx += t1_ct;
                idx += t1_ct;
            }  
            ...
            if(idx != row_lim[r])  active_row++;       
            row_lim[r] = idx;
            
        }
        lastIdx_tile[tile_no] = sp_idx; 
    }
    new_nnz = sp_idx;
    delete(row_lim);
    cout <<" from sp:  nnz " << sp_idx << " " << dp << endl;
}
// void rewrite_dp_matrix(int * row_ptr, int * row_ind, int *col_ind, float * val_ind, 
//     int *dp_rows, int *dp_cols, float * dp_vals, long nnz, long n_rows, long n_cols,
//     int TS, int *tiled_ind, int * lastIdx_tile,  int block, long &new_nnz){
//     int th = 5;
//     long dp_idx = 0, sp=0, idx =0;
//     TS = TS/2;
//     int n_tile = n_cols/TS + 1, tile_no=0;
//     long *row_lim = new long[n_rows];
//     lastIdx_tile[0] = 0; 
...
//                 idx -= t1_ct;
//                 for (int i = 0; i < t1_ct; ++i){
//                     tiled_ind[dp_idx+i] = idx+i;
//                     dp_rows[dp_idx+i] = row_ind[idx+i];
//                     dp_cols[dp_idx+i] = col_ind[idx+i];
//                     dp_vals[dp_idx+i] = val_ind[idx+i];
//                 }
//                 dp_idx += t1_ct;
//                 idx += t1_ct;
//             } 
...
//             row_lim[r] = idx;            
//         }
//         lastIdx_tile[tile_no] = dp_idx; 
//     }
//     new_nnz = dp_idx;
//     delete(row_lim);
//     cout <<"from dp: nnz " << sp << " " << dp_idx << endl;
// }


int rewrite_dp_matrix(int * row_ptr, int * row_ind, int *col_ind, float * val_ind, 
    int *dp_rows, int *dp_cols, float * dp_vals, long nnz, long n_rows, long n_cols,
    ...
    for(int tile_lim = TS; tile_lim <= (n_cols+TS-1); tile_lim+=TS){ 
        ...
        for(int r = 0; r <n_rows; ++r){ 
            ...
            if(t1_ct > th){
                ...
                for (int i = 0; i < t1_ct; ++i){
                    tiled_ind[dp_idx+i] = idx+i;
                    dp_rows[dp_idx+i] = count[tile_no-1]; //row_ind[idx+i];
                    dp_cols[dp_idx+i] = col_ind[idx+i];
            ...
            if(cur_block >= sh_tile_r) { 
                cur_block = 0;
                lastIdx_block_tile[(tile_no-1)* max_block_inAtile + block_count] = dp_idx; 
                block_count++;  
