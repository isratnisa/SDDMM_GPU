__global__ void dense_comp_kernel_COO(int const* __restrict__ row_ind, int const* __restrict__ col_ind, float *val, const float * __restrict__ u, const float * __restrict__ v, 
  ...
  int *no_block_tile, int max_active_block, int tile_size, int max_active_row){
    ...
    __shared__ float sh_c[3072 * 1];
    __shared__ float sh_r[3072 * 1];
    ...
    int step = blockDim.x >> 5 * WARP_SIZE;
    unsigned int c = tId + tile_st;
    int tile  = blockIdx.x;// col/tile_size;
    int t = tid_in_WARP;

    for (int inner_t = 0; inner_t < k; ++inner_t){       
        for (int i = WARP_ID * WARP_SIZE; i < sh_tile && i+t < n_cols; i+=step)
            sh_c[i + t] = v[ inner_t * n_cols + (tile * sh_tile + i + t) ]; //v[(tile * sh_tile + i) * k + t];                          
        __syncthreads();

        int g_n_blocks = no_block_tile[blockIdx.x]; 
        int n_blocks=0;
        ...
        for (int block = 0; block < n_blocks ; block++){          
            ...
            // WARP_ID = tId >> 5;// tid_in_WARP = tId & 31;
            step = blockDim.x >> 5;       // int t = tid_in_WARP;
            for (int i = WARP_ID; i < sh_tile_r/32 && i*WARP_SIZE+t < n_rows; i+=step)
                sh_r[i * WARP_SIZE + t] = u[ inner_t * n_rows + active_row[(tile*max_active_row) + (block * sh_tile_r) + (i* WARP_SIZE + t) ]];                  
            __syncthreads();
            ...
            for (int c = (tId >> 0) + block_st; c < block_end && c< nnz; c+=(blockDim.x >> 0)){
                ...
                int sh_row = row - block * sh_tile_r;
                int sh_col = col - tile * sh_tile;
                //sm = u[inner_t *n_rows+active_row_] * sh_c[ sh_col];
