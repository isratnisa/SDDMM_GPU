...
int BLOCKSIZE=1024;
int DGEMM_load = 32;
int block_th =  BLOCKSIZE * DGEMM_load;
...
int * count_actv_row, int &max_active_block, int *d_no_block_tile, long new_nnz, int &max_active_row, float *d_p, int * d_tile_block_rowSt){
    ...
    max_active_block, tile_sizeX, max_active_row, d_p);
        ...
        //change orientation
        comp_kernel_COO_DGEMM_2<<<grid, block, 0, stream[0]>>>(d_row_ind, d_col_ind, d_val_ind, d_W, d_H, 
    new_nnz, n_rows, n_cols, k, d_active_row, d_lastIdx, d_lastIdx_block_tile, d_no_block_tile, 
    max_active_block, tile_sizeX, max_active_row, d_p, d_tile_block_rowSt);
...
void init(int *rows, int *cols, float* vals){
    ...
    int n_tile_r = n_rows/actv_row_size + 1;
    int max_active_block = (n_rows*actv_row_size/block_th+1);
    int *count_actv_row = new int[n_tile_c];
    int *no_block_tile = new int[n_tile_c];
    int *lastIdx_tile = new int[n_tile_c+1];
    int *tile_block_rowSt = new int[n_tile_c * max_active_block];
    int *lastIdx_block_tile = new int[(n_tile_c+1) * (max_active_block+1)];
    float *d_val, *d_W, *d_H, *d_W_t, *d_p;
    int *d_row_ptr, *d_col_ind, *d_row_ind, *d_tiled_ind, *d_lastIdx, *d_active_row, *d_lastIdx_block_tile, *d_no_block_tile, *d_tile_block_rowSt;
    ...
    max_active_row = rewrite_matrix_1D(row_ptr, rows, cols, vals, new_rows, new_cols, new_vals, nnz, n_rows, n_cols, 
        ...
        no_block_tile, actv_row_size, tile_block_rowSt, block_th);
  ...
  //***********Starting GPU****************
    ...
    checkCuda(cudaMalloc((void**)&d_tile_block_rowSt, n_tileX*max_active_block*sizeof(int)),4);
    ...
    for (int i = 0; i < n_tileX; ++i)
        checkCuda(cudaMemcpy(d_tile_block_rowSt+i*max_active_block, &(tile_block_rowSt[i*max_active_block]), max_active_block*sizeof(int), cudaMemcpyHostToDevice),4);  
    ...
    d_lastIdx_block_tile, d_active_row, count_actv_row ,max_active_block, d_no_block_tile, new_nnz, max_active_row, d_p, d_tile_block_rowSt );
    //******** correctness check
